...
// MKS_GEN_L
  ...
  case 0: return (PORTF & 2)?0:1; 
  case 1: return (PORTF & 128)?0:1; 
  case 2: return (PORTL & 2)?0:1; 
  case 3: return (PORTA & 64)?1:0;  
#endif
	}
	return 0;
}
...
// MKS_GEN_L
#elif (MOTHERBOARD == BOARD_MKS_GEN_L)
  case 0: if (!dir) PORTF |= 2; else PORTF &= ~2; break;
  case 1: if (!dir) PORTF |= 128; else PORTF &= ~128; break;  
  case 2: if (!dir) PORTL |= 2; else PORTL &= ~2; break;    
  case 3: if (dir) PORTA |= 64; else PORTA &= ~64; break; 
#endif
	}
	asm("nop");
}
...
// MKS_GEN_L
  ...
  if (portF & 128) dir_bits |= 2; 
  if (portL & 2) dir_bits |= 4; 
  if (portA & 64) dir_bits |= 8;  
  dir_bits ^= 0x07; //invert XYZ, do not invert E
#endif
	return dir_bits;
}
